...
file-system tree (sorted by key = obj-id, then type):
- `(D, DIR_REC, hash(f))` → directory entry record pointing to the file's object identifier
...
- **M2 — trees.** Nested directories and multiple files; per-directory child
  counts; file-system tree records correctly sorted; grow the file-system tree to multiple
